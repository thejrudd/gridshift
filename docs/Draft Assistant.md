...
- The v8.2 feature tour follows the same draft-status presentation: before the draft it explains Picks, then previews the future Results board with tour-only placeholder teams and players. After the draft starts it preserves real league results. Both paths explain that the league-year selector opens past draft results.
