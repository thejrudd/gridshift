...
- Live pick progress uses the first unfilled slot in the draft order, not the count or highest number of returned picks: Sleeper can preassign keepers in later rounds. War Room, Board, Results, and recommendation timing share `buildUpcomingDraftWindow`; upcoming turns and picks-away counts exclude occupied keeper slots while keeper players remain unavailable.
...
- Highlight conflicts compares each Board candidate only with players currently locked to the manager's roster, including the manager's completed picks in the active draft. Saved Board targets are never comparison peers; use the `Bye` card metric to inspect their individual bye weeks.
- Any same-week overlap with a currently rostered player is a conflict. An overlap at the same normalized exact position receives stronger severity; a different-position overlap remains a standard warning even when the players share FLEX, SUPER_FLEX, or IDP flex eligibility. Shared flex compatibility never elevates severity.
...
12. Drafted players are excluded from the recommendation candidate pool, so `index.js` separately enriches each drafted player into `draftedCardsById` (a `Map` keyed by Sleeper player ID) using the same signal builders. This feeds the Results view and lets All Players show currently rostered drafted/keeper players without changing recommendation order.
...
- `War Room` and `Board` render their full tools while Sleeper reports the selected draft as `pre_draft`, `drafting`, `in_progress`, or `paused`, so both stay usable through a live draft room. They share one availability predicate in `DraftBoardDataView`; do not reintroduce a separate War Room gate. For completed or otherwise inactive drafts, including historical post-draft league years, the War Room route remains selected and revisitable but shows a centered, season-aware unavailable explanation until the user chooses another Draft view or returns to a league year whose draft has not finished. `Results` stays active before, during, and after the draft.
...
- Player scope keeps the ranked display responsive: Available shows the capped ranked window of unrostered players from Sleeper's draft pool, while All Players preserves every current rostered candidate in addition to that window, including rostered players outside a rookie/veteran-only pool and rostered players already recorded as keeper/draft picks. Previously recorded picks remain reference-only and cannot be added to the board. Rookies remains an explicit rookie-only scope.
