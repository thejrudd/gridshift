...
The v8.5 Statistics Scores Beta combines a development/test-only regular-season design fixture with server-proxied BALLDONTLIE live data and an explicit ESPN fallback. With a verified paid profile and effective request limit, BALLDONTLIE owns the selected week's near-live score, game state, period, and clock anchor as well as richer game drilldowns. ESPN remains the broadcast/network metadata source for BALLDONTLIE-backed games because BALLDONTLIE does not expose that field; the metadata overlay does not replace BALLDONTLIE score or live-state data. Without a usable BALLDONTLIE games lane, ESPN supplies the scoreboard slate. Fantasy Live league readiness does not control either source. In production, the optional StoryStats sidecar automatically warms primetime game stories; development keeps the Game Story section as a manual test surface.

## Route And Ownership

- Route: `/statistics/scores`
- Canonical share state: `/statistics/scores?season=2026&phase=preseason&week=2&game=1393548&tab=players&group=rushing`; the phase is explicit so a shared link is not changed by the recipient's local preseason preference, while Overview and unused player-category defaults are omitted
- Route parser and canonical URL builder: `src/utils/appRoutes.js`; `game` is the provider-stable matchup ID, `tab` is the drilldown section, and `group` is the optional Players category
...
- Shared BALLDONTLIE play-by-play normalization (`normalizeBdlScorePlay()` is a thin adapter over it): `src/utils/playByPlay/normalizePlay.js` — [[Play-By-Play Normalization]]
...
- Inline live latest-play row: `src/components/statistics/scores/LatestPlayStrip.jsx`; its data arrives in the canonical selected-week live snapshot
- Game drilldown: `src/components/statistics/scores/ScoresGameDrilldown.jsx`
- StoryStats request boundary: `server/storyStatsHandlers.js`; browser contract: `src/api/statisticsScoresApi.js`
- StoryStats presentation: `src/components/statistics/scores/StoryStatsPanel.jsx`
...
In development and test environments, the desktop default is the Claude **Hero week + peek** design: the selected week starts with one **Live** section for live, halftime, and delayed games, followed by calendar-day sections in chronological order. Calendar headings use the NFL's `America/New_York` day boundary so UTC kickoff timestamps cannot place Friday games under Thursday. Scheduled scorebug metadata instead formats the raw kickoff in the user's browser timezone and includes that timezone's short label; this keeps local dates and times unambiguous even when a game crosses the NFL calendar-day boundary. Compact Previous and Up Next week summaries follow the selected slate. Mobile shows only the selected week beneath the shared horizontal week rail. Production does not bundle or display the placeholder slate.

The drilldown contains Overview, Team Stats, Players, Scoring, Play-by-Play, and an optional **Game Story** section. The selected season, phase, week, matchup, drilldown tab, and Players category are reflected in the canonical Scores query state, so a copied URL restores the same matchup context after a reload. Entering a different matchup resets the page to the top, while background scoreboard refreshes preserve the reader's scroll position within the current matchup. Scheduled BALLDONTLIE games are drilldown-openable so the StoryStats pregame window can be exposed before kickoff. Player statistics use full tables on wider screens and priority cards on mobile. For provider-backed games, the Scores sidecar aggregates BALLDONTLIE's game, team-stat, player-stat, and play responses into the same normalized detail contract used by the development fixture. BALLDONTLIE play rows are grouped into display drives by the team that ran each play; the provider supplies individual plays rather than a documented drive object. See Drive Grouping below. ESPN-only scorebugs remain visible and live-updating but do not expose a drilldown because ESPN does not supply the required detail contract.

Player rows include height and weight from the BALLDONTLIE player object when supplied. Height is displayed in feet and inches and weight in pounds, beneath the player's name and position in both the wide table and mobile cards. Missing measurements are omitted.

### StoryStats Beta

StoryStats is an optional narrative sidecar for BALLDONTLIE-backed NFL games. The Game Story section always requests the fixed `audience=stats` and `tone=editorial` variant. GridShift does not request, display, or use StoryStats betting content, odds, props, predictions, or wagering recommendations. The story is editorial context only; score, clock, fantasy points, and play-by-play remain owned by the existing Scores data paths.

The beta follows the provider's documented availability windows and conserves the free allowance:

- **Pregame:** one request only when the scheduled kickoff is within one hour. Games farther out wait locally and do not spend a request.
- **Live:** one request per observed regulation-period transition, up to four live requests per game. The response is cumulative, so the panel deduplicates stable story IDs and does not poll on the eight-second play cadence. A live story is its own drilldown section and does not conflict with the canonical play feed.
- **Postgame:** one request after the score status becomes final. A provider 404 is treated as “not generated yet” rather than as a score or play failure.
- **Budget:** the default guard is 10 StoryStats requests per API key per UTC day. Requests are per game-scoped endpoint call; a pregame request for three games costs three requests, not one. The server keeps StoryStats credentials and the beta ledger out of the browser. The current beta ledger is process-local and resets if the API sidecar restarts.

Production automation discovers regular-season primetime games from the BALLDONTLIE schedule, using the existing 7:00 PM Eastern kickoff rule. It warms the pregame story within one hour of kickoff, requests one live story for each observed regulation period (up to four), and requests one postgame story after final. The scheduler shares the StoryStats service cache and daily guard with the browser route, and uses a game-scoped BDL status refresh while a primetime game is active. The current beta assumes one sidecar process; scheduler progress is process-local and can repeat an in-window phase after a restart.

In development, the scheduler is disabled even when `GRIDSHIFT_STORY_STATS_AUTOMATION_ENABLED` is present. The user-triggered beta fetches when the user selects the **Game Story** section for a BALLDONTLIE game drilldown. Opening a game for score, box-score, or play-by-play detail does not spend StoryStats budget. This keeps the dev environment suitable for manual testing without silently warming the entire schedule.

Live, halftime, and delayed BALLDONTLIE scorebugs add a compact **Latest play** row directly beneath the scorecard. It uses the same normalized narrative, spot, down-and-distance, clock, outcome tags, and possession correction as the drilldown feed, including penalties, interceptions, fumbles, punts, kickoffs, and turnover-on-downs. The trajectory strip and player expansion are intentionally omitted at this density; the scorebug remains the primary interaction. ESPN score-only cards do not fabricate a play row.

Statistics Scores keeps its Regular / Preseason control. Statistics Schedule instead starts with regular-season weeks only and exposes an unchecked **Include preseason** control in **By Week**; enabling it prepends the four ESPN preseason slates to that week rail. **By Team** stays regular-season-only and does not show the preseason control. PrimeTime is not offered while a preseason week is selected. Scores advances preseason slates at the start of the next slate's first NFL calendar day in `America/New_York`; for a connected league's current season, the regular-season `Now` marker follows the shared fantasy `currentFantasyWeek`, while public/no-league and historical views retain the provider kickoff fallback. This keeps the Scores rail aligned with Fantasy Schedule, Matchups, Live, and the other fantasy surfaces without changing public-season history behavior. The week rail retains the selected underline and current-week “Now” metadata without repeating that state in a separate toolbar control. Provider-backed scorebugs open the real-data detail shell in both Regular and Preseason for scheduled, final, live, halftime, and in-game delayed states; scheduled games are enabled so pregame stories can be read before kickoff. Postponed, partial, offline, and unavailable states remain non-interactive. Local fixture detail remains regular-season only and follows the same game-state boundary.
...
- BALLDONTLIE supplies the live clock through its game snapshot and the more granular `period`/`clock_display` fields on its newest play. The sidecar resolves those into one canonical live-game snapshot and never permits an older event to move game progress backward. The UI displays only the latest provider-verified clock; it does not synthesize a running clock between provider updates because the provider does not document a clock-running flag.
...
- The selected-week live response includes each eligible BALLDONTLIE game's canonical newest play. Scorecards do not start a second play poll. Statistics drilldown and Fantasy Live play routes read the same sidecar-wide play snapshot, cache identity, and in-flight request while retaining their separate public/league authorization boundaries.
- The normalized latest-play response also carries its post-play away/home score. When that response is fresher than the score snapshot, the scorebug reconciles the displayed score from the play even if the play's game-clock value is earlier; stale or older play responses cannot roll the score back.
- An open BALLDONTLIE live, halftime, or delayed drilldown refreshes its response envelope every eight seconds. The sidecar keeps team and player resources on their longer cache while projecting the same canonical eight-second play snapshot used by scorecards and Fantasy Live. Refresh pauses while the page is hidden or offline and retains the last good box score after a transient failure.
...
- StoryStats uses the separate `GRIDSHIFT_STORY_STATS_API_KEY` through the same server-side gateway. The handler sends only `audience=stats` and `tone=editorial`, keeps the raw narrative bounded, and reports the daily beta allowance without exposing credentials.
...
- ESPN scoreboards do not provide the play contract used by the inline feed, so ESPN fallback cards remain score-only rather than showing invented or stale provider plays.
...
| **StoryStats automatic mode** | Add `GRIDSHIFT_STORY_STATS_API_KEY` to the production API environment and leave `GRIDSHIFT_STORY_STATS_AUTOMATION_ENABLED=true`. | The production API sidecar discovers regular-season primetime games and warms pregame, period-based live, and postgame stories. The browser reads the warmed cache from the Game Story section; it does not initiate an upstream StoryStats request in production. |

For local BALLDONTLIE testing, put the keys only in `/Users/justinruddick/Documents/GitHub/gridshift/.env`:

```dotenv
GRIDSHIFT_BDL_API_KEY=replace_with_your_local_key
GRIDSHIFT_BDL_TIER=goat
GRIDSHIFT_BDL_EFFECTIVE_MAX_REQ_PER_MIN=600
GRIDSHIFT_STORY_STATS_API_KEY=replace_with_your_storystats_key
GRIDSHIFT_STORY_STATS_DAILY_LIMIT=10
GRIDSHIFT_STORY_STATS_AUTOMATION_ENABLED=true
```

`scripts/dev.mjs` reads that repo-root `.env` into the server process. It does not read `.env.local` for server configuration. Neither key must be named with a `VITE_` prefix, added to client source or public assets, stored in browser storage, or sent from browser code to BALLDONTLIE. Vite-prefixed variables are exposed to the client bundle. Keep the committed `.env.example` values blank and never commit a real key. Restart `npm run dev` after changing either key. `GRIDSHIFT_STORY_STATS_AUTOMATION_ENABLED` is intentionally ignored for background execution in development; it only controls the production scheduler.
...
| Provider clock parsing, monotonic corrections, and provider-anchor metadata used at the normalization boundary. | `tests/unit/providerAnchoredGameClock.test.mjs` | `node --test tests/unit/providerAnchoredGameClock.test.mjs` |
...
| The local source selector keeps Fixture provider-free, preserves the ESPN eight-second path, receives game clock and Latest Play together from the BALLDONTLIE live-week route, keeps the verified clock unchanged between provider snapshots, preserves provider game identity for a preseason drilldown, reproduces the Aug. 13 rollover, and stays contained on a narrow phone. | `tests/e2e/statistics-scores-sources.spec.js` | `npx playwright test tests/e2e/statistics-scores-sources.spec.js` |
...
The Players tab keeps the provider's full-game totals as the authoritative box score. Each player row is expandable; when the detail response contains confidently parsed play text, the expanded row shows a quarter-by-quarter split derived from those plays, including a Total row for comparison. Metrics the play feed cannot identify safely remain `—`, and games without usable play coverage explain why the split is unavailable. Player rows show the team mark alongside the abbreviation, with the mark occupying a stable slot if the remote logo fails. Each row also shows the player's headshot and a position chip (the shared `LivePosChip` Fantasy uses). The position comes from BALLDONTLIE's `position_abbreviation`, falling back to the ESPN participant record; the headshot comes from the same per-game ESPN participant index as the play feed (see Where the photos come from below), so a failed or unresolved photo drops to initials rather than repeating the team mark. Every stat column header is sortable: selecting a new metric starts with highest-first leader order, selecting it again reverses the direction, and unknown values remain below known values. The narrow card layout mirrors this with a compact Sort by control.
...
- The drive mix counts an interception once, as the pass attempt it was. The pass total carries an `INT` modifier (for example, `11 pass (1 INT)`) so the turnover is explicit without inventing an eighteenth play.
...
The live scorecard preview is the same play sentence and outcome vocabulary without the trajectory strip or expandable breakdown. Its possession-change label is derived from the same corrected provider team semantics as drive grouping, so a punt, turnover, or fourth-down stop cannot be attributed to the wrong side.
...
The play parsing, name matching, field geometry, and field/momentum graphics live in `src/utils/nflPlays/` and `src/components/nflPlays/` rather than under `statistics/`, because Fantasy Live solves the same problems. Provider-row semantics — the canonical play shape and the offense/defense rule — are shared through `src/utils/playByPlay/`: [[Play-By-Play Normalization]]. `PlayCard.jsx` is the only Statistics-specific piece.
...
- An **interception** is the only play that reverses direction mid-flight, so the throw, the moment it is picked off, and the return the other way are three separate movements around a spot only the description reports. Drawn as one line it was just a long gain in the wrong colour. The NFL gamebook can express an end-zone catch as a signed spot (`SEA -3` means three yards inside Seattle's end zone) and can insert a bracketed defensive attribution between the interceptor and that spot; both forms remain part of the same shared spot grammar. That authoritative description is sufficient to choreograph the pass even when the provider omits its compact `short_text` summary or supplies a form the narrative grammar does not recognize. A touchback ends at the catch rather than animating an invented return to the next possession's placement. A forward pass is always drawn forward: the feed sometimes credits the pick at a spot *behind* the line of scrimmage, and throwing the ball there sends it the way the offense came from, which reads as the entire play running backwards. When the reported spot is behind the line the ball is thrown to a target estimated from the depth the description states, the spot stops being spoken — the same rule the catch point follows — and the return still finishes where the feed says it did, so field position stays true. Only the return may reverse direction. A compact scoring-summary row may name only the returner. The shared sequence context infers its passer only from a confidently parsed pass earlier in the same possession, so Statistics Scores and Fantasy Live use the same evidence; it derives the pick spot from the return distance and final field boundary and keeps the passer anonymous when that evidence is unavailable.
...
`fieldGeometry.js` works in one absolute frame: a percentage across the field with the away team's goal line at 0 and the home team's at 100, so the away team always drives left to right. `fieldSpots.js` extends that frame through the two end zones (`-10..110`) for signed gamebook positions without changing ordinary on-field coordinates.
...
- Keep the canonical Scores query state limited to durable context; transient drive expansion and play-feed filters remain local to the drilldown.
