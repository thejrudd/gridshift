...
Fantasy Matchups uses the server-only `/nfl/v1/fantasy/projections` endpoint when the configured BDL capability profile includes `fantasy` (GOAT or an equivalent trial profile). The sidecar validates the season/week, paginates and caches the weekly snapshot, and returns only the identity, game, projected-stat, and sanitized freshness fields needed by the browser. GridShift recalculates projected points from the provider stat line under the connected league's scoring rules, using BDL's precomputed format total only when the raw stat line cannot produce a usable local score. BDL remains the primary external projection source; when it is missing, unsupported, unavailable, or cannot be matched unambiguously to the connected player directory, Sleeper's public weekly projection stat line is used next and scored with the connected league's settings. Matchups then falls back to its current-season model and—at the opening of a season—the prior completed Sleeper season; no BDL response is required for the page to remain usable. The completed-matchup header comparison sums the available projected scores from the visible non-bench starter rows, preferring a complete recorded pregame set and then the current BDL, Sleeper, or GridShift-derived row values; it is omitted only when no starter projections are available.

The server keeps only provider rows whose season and week exactly match the requested weekly snapshot. A season-level row or a row from another week/season is rejected before it can be scored as one game's projection.
