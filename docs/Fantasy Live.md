...
- `src/utils/livePlaysFeed.js` — play-by-play matching, per-play stat deltas, shared feed ordering, and the 0..1 per-game progress value (`getPlayProgress()`); routes its name index through `nflPlays/playerNameIndex.js`.
- `src/utils/livePlaySplitting.js` — turns an aggregate stat change between polls into separate fantasy-valued play stat lines.
- `src/utils/liveReconciliation.js` — the pure engine that reconciles those plays against Sleeper: `reconcileLivePlays(previousState, inputs)` returns each player's `sleeperPoints`, `pendingPoints`, `displayedPoints`, `adjustment`, their plays with `status`/`displayPts`, any `stat-update` fallback rows, and one flat newest-first `feedEvents` list. No clock reads and no I/O — `now` comes from the caller, so replay steps it on the slate clock and connected live on wall time.
...
The production chart and feed use a **kickoff-ordered slate axis**, not a raw per-game clock or refresh timestamp: every relevant game receives one consecutive 0..1 segment in kickoff order. This keeps a late live game after an earlier completed game and gives feed navigation one direction through the slate. `getPlayProgress()` (`src/utils/livePlaysFeed.js`) still records where a play happened inside its own game for pace and win-probability calculations; `CompanionLive` maps it to the shared axis before `buildPaceSeries()` (`src/utils/livePace.js`) builds the cumulative scoring curve and milestones. Solid actual-score paths connect each team's scoring dots directly, including distinct same-moment changes, and hover continuously samples the visible segment between those dots; win probability remains independently interpolated. Every fantasy-relevant provider play or split snapshot-delta play gets its own feed row. A shared NFL snap can therefore produce separate rows for each rostered contributor, and zero-value/non-fantasy changes are omitted; the chart consumes those same individual rows. Connected BDL plays are reconciled against Sleeper, which never writes a row of its own. Each play is scored from its own parsed stat delta with the league's scoring settings, and a play stays **pending** until Sleeper's own numbers account for it — by stat line where a fresh one exists, otherwise by points — at which point it becomes **confirmed**. Pending rows carry a small marker; confirmed rows carry none. A play's own `pts` is never rewritten: the residual (Sleeper's points minus the confirmed plays' points) is pinned to that player's latest confirmed play as a separate `adjustment`, and `displayPts = pts + adjustment` is what the feed headline, the chart and the hero consume. The residual may be negative, because stat corrections are. **Displayed player total = Sleeper's last `players_points` + Σ pts of the plays Sleeper has not covered yet**; the side total is the sum of those, and the hero, performer rail, pace chart, verdict, player sheet and feed header all read that one number. Only the settled final-official branch replaces it with Sleeper's own team total. One fallback survives: when Sleeper's stat line shows stats no play explains — free tier, a parser miss, a coverage gap — a single `source: 'stat-update'` row per unexplained batch appears after a grace period, labelled "Stat update", and is retired if a matching play arrives later. Replay prefers a complete persisted v3 snapshot recorded at or before the selected play. When none exists, the canonical engine reconstructs each starter independently from the points observed by that moment and that starter's own kickoff/game-progress timeline. `livePace.js` must not derive a second probability formula. Only the final chart point receives the current authoritative score and live snapshot directly, keeping the chart, hero, verdict, and explainer identical; in replay that close must equal the accumulated event total so it cannot introduce a corrective wall at NOW. Feed anchoring uses the same axis (`anchorProgress`), so any new event source must carry a `progress` value or it cannot be placed. Plays carry their own game progress and are mapped to the slate segment; a stat-update row has no clock at all, so `CompanionLive` falls back to the starter's team → game → pace progress before it can be anchored.
